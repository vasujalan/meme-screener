...
def find_model_files():
    ...
    dirs_to_check = [
        ...
        "/mount/src/meme-screener",
        "/mount/src/meme-screener/models",
    ]
    for d in dirs_to_check:
        sp = os.path.join(d, "scaler.pkl")
        xp = os.path.join(d, "xgb_best.json")
        if os.path.exists(sp) and os.path.exists(xp):
            return sp, xp
    return None, None
...
def load_models():
    ...
    scaler_path, xgb_path = find_model_files()
    if scaler_path is None:
        return None, None, "Model files not found. Ensure scaler.pkl and xgb_best.json are committed to your GitHub repo."
    try:
...
# Seed keywords used to discover trending tickers from Google Trends
TREND_SEEDS = [
    "short squeeze", "meme stock", "most shorted stock",
    "stock to buy", "next gamestop", "reddit stock",
    "wallstreetbets", "buy stock now", "hot stock",
    "penny stock", "small cap stock", "trending stock",
]

# Fallback list used ONLY if Google Trends returns nothing
FALLBACK_TICKERS = [
    "GME","AMC","KOSS","MVIS","QUBT","BB","NOK",
    "FFAI","DUO","SPCE","BYND","PLUG","HOOD","SOFI",
]

# Known valid US stock tickers to match against trend queries
KNOWN_TICKERS = {
    "GME","AMC","KOSS","BB","NOK","MVIS","QUBT","FFAI","DUO","DJT",
    "SPCE","NKLA","RIDE","BYND","PLUG","OPEN","HOOD","SOFI","UWMC",
    "PLTR","NVAX","SNDL","CLOV","WKHS","EXPR","TSLA","NVDA","AAPL",
    "MSFT","META","AMZN","GOOGL","NFLX","AMD","INTC","SNAP","UBER",
    "LYFT","RIVN","LCID","NIO","XPEV","BBAI","SOUN","IONQ","RGTI",
    "SMCI","MSTR","COIN","RIOT","MARA","CLSK","HIMS","CHWY","FIVE",
    "BBBY","CLOV","WKHS","EXPR","WISH","CLOV","SKLZ","LIDR","GFAI",
}


def discover_trending_tickers(status_placeholder=None) -> list:
    """
    Query Google Trends for stock-related rising searches.
    Extract ticker symbols from the results.
    Returns a deduplicated list of tickers sorted by trend strength.
    """
    def update(msg):
        if status_placeholder:
            status_placeholder.markdown(msg)

    discovered = {}   # ticker -> trend score (higher = more trending)
    ...
    try:
        ...
        update("🔍 Scanning Google Trends for trending stocks...")

        for seed in TREND_SEEDS[:8]:   # limit to 8 seeds to avoid rate limiting
            try:
                ...
                for df_q, weight in [(rising, 2), (top, 1)]:
                    if df_q is None or df_q.empty:
                        continue
                    for _, row in df_q.iterrows():
                        query = str(row.get("query", "")).strip().upper()
                        # Direct ticker match (1-5 uppercase letters)
                        words = re.findall(r"\b([A-Z]{1,5})\b", query)
                        for w in words:
                            if w in KNOWN_TICKERS:
                                score = int(row.get("value", 50)) * weight
                                discovered[w] = discovered.get(w, 0) + score

            except Exception:
                time.sleep(3)
                continue

        # Also check US trending searches for any ticker mentions
        try:
            pt2 = TrendReq(hl="en-US", tz=360)
            trending = pt2.trending_searches(pn="united_states")
            if not trending.empty:
                for query in trending[0].values[:30]:
                    words = re.findall(r"\b([A-Z]{1,5})\b", str(query).upper())
                    for w in words:
                        if w in KNOWN_TICKERS:
                            discovered[w] = discovered.get(w, 0) + 30
        except Exception:
            pass

    except Exception as e:
        update(f"⚠️ Google Trends unavailable: {e}")

    if not discovered:
        update("⚠️ No trending tickers found from Google Trends — using fallback list.")
        return FALLBACK_TICKERS

    # Sort by trend score, return top 20
    sorted_tickers = sorted(discovered.items(), key=lambda x: x[1], reverse=True)
    result = [t for t, _ in sorted_tickers[:20]]

    update(f"✅ Found **{len(result)} trending tickers** from Google Trends: {', '.join(result)}")
    return result
...
# ── TAB 1: AUTO SCREENER ──────────────────────────────────────────────────────
with tab1:
    st.markdown("### Live Trending Stock Screener")
    st.markdown(
        "Automatically discovers stocks **currently trending on Google** "
        "using stock-related search queries, then scores each one through the meme episode model."
    )

    extra_input = st.text_input(
        "Add extra tickers to include (optional, comma-separated)",
        placeholder="e.g. TSLA, RIVN, BBAI",
    )
    run_auto = st.button("🚀 Discover & Screen Trending Stocks", key="auto")

    with st.expander("ℹ️ How this works"):
        st.markdown("""
        1. Searches Google Trends for rising queries related to: *short squeeze, meme stock, reddit stock, most shorted stock*, and more
        2. Extracts any stock ticker symbols found in the rising search results
        3. Scores every discovered ticker through the trained XGBoost model
        4. Returns a ranked table — highest meme probability at the top

        If Google Trends returns no results (e.g. rate limited), a fallback list of known meme-susceptible stocks is used.
        """)

    if run_auto:
        st.markdown("---")
        discover_status = st.empty()
        tickers = discover_trending_tickers(discover_status)
        ...
        if not tickers:
            st.warning("Could not discover any tickers. Try adding tickers manually in the Search tab.")
            st.stop()

        st.info(f"📡 Discovered **{len(tickers)} tickers** from Google Trends: {', '.join(tickers)}")
